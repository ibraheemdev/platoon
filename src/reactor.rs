...
impl Reactor {
    ...
    pub async fn ready(&self, key: usize, direction: Direction) -> io::Result<()> {
        struct Ready<'a> {
            sources: &'a UnsafeCell<Sources>,
            poller: &'a polling::Poller,
            tick: &'a Cell<usize>,
            key: usize,
            direction: Direction,
        ...
        impl<'a> Future for Ready<'a> {
            ...
            fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
                ...
                let source = unsafe { (*self.sources.get()).get_mut(&self.key).unwrap() };
                ...
                let waker = self.waker.get_or_insert_with(|| {
                    ...
                    ticks = Some([current_tick, interest.last_tick]);
